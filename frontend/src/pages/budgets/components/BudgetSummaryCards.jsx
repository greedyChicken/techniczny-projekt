...
import { budgetSummaryCardStyles, budgetSummaryGridSx } from "../styles/budgetStyles";
...
const BudgetSummaryCards = ({ totalBudget, totalSpent, overBudgetCount }) => {
    ...
    return (
        <Box sx={budgetSummaryGridSx}>
            {SUMMARY_ITEMS.map((item) => {
                const Icon = item.icon;
                const value = values[item.key];
                return (
                    <Card key={item.key} sx={budgetSummaryCardStyles.card}>
